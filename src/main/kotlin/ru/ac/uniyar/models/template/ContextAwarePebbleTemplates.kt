...
fun Caching(baseTemplateDir: String): ContextAwareTemplateRenderer {
    ...
    return ContextAwarePebbleTemplateRenderer(configure(PebbleEngine.Builder().cacheActive(true).loader(loader)).build())
}

fun HotReload(baseTemplateDir: String): ContextAwareTemplateRenderer {
    val loader = FileLoader()
    loader.prefix = baseTemplateDir
    return ContextAwarePebbleTemplateRenderer(configure(PebbleEngine.Builder().cacheActive(false).loader(loader)).build())
}
}